        ...
        <div class="rounded-2xl border border-gray-200 bg-white shadow-sm">
            <div class="relative aspect-[16/10] w-full overflow-hidden rounded-t-2xl bg-gray-100">
                ...
                <div class="absolute top-3 left-3 flex items-center gap-2">
                    <span
                        class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold ring-1 ring-inset {{ $badgeFor($house->energy_rating) }}">BER
                        {{ $house->energy_rating }}</span>
                    <span
                        class="inline-flex items-center rounded-full bg-white/90 px-2.5 py-1 text-xs font-medium text-gray-700 ring-1 ring-gray-200 backdrop-blur">{{ $prettyType }}</span>
                </div>
            </div>
                ...
                <div class="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-2">
                    ...
                    <div class="rounded-xl border border-gray-200 p-4">
                        ...
                        <dl class="grid grid-cols-2 gap-x-4 gap-y-2 text-gray-800">
                            ...
                            <dd>{{ $prettyType }}</dd>
                            <dt class="text-gray-500">Energy</dt>
                            <dd>BER {{ $house->energy_rating }}</dd>
                            ...
                            <dd>{{ $house->created_at?->format('Y-m-d H:i') }}</dd>
                            <dt class="text-gray-500">Updated</dt>
                            <dd>{{ $house->updated_at?->format('Y-m-d H:i') }}</dd>
                        </dl>
                    </div>
                </div>
